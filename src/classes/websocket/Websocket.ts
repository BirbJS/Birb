  ...
  private async onPacket(body: any): Promise<void> {
    ...
    switch (body.op) {
      ...
      case PacketOperation.RECONNECT: {
        this.client.debug(
          'discord has requested that we reconnect (the gateway is probably restarting); closing this connection to resume...'
        );
        this.close(1000);
        await wait(100);
        this.reconnect();
  ...
  private async onClose(code: number): Promise<void> {
    ...
    switch (code) {
      ...
      case GatewayCloseCode.DECODE_ERROR: {
        ...
        console.error(
          new WebsocketError('websocket closed due to an unknown opcode or decoding error; is Birb.JS is up-to-date?')
        );
        break;
      }
      ...
      case GatewayCloseCode.SHARDING_REQUIRED: {
        ...
        console.error(
          new ShardingError(
            "you must shard your Discord bot due to the amount of servers it's in\nHead to: https://birb.js.org/explained/sharding"
          )
        );
        this.preventReconnect(true);
        break;
      ...
      case GatewayCloseCode.INVALID_INTENTS: {
        ...
        console.error(
          new WebsocketError('the intents provided are invalid\nHead to: https://birb.js.org/explained/intents')
        );
        this.preventReconnect(true);
        break;
      }
      case GatewayCloseCode.DISALLOWED_INTENTS: {
        ...
        console.error(
          new WebsocketError(
            'you do not have authorization to use the intents provided\nHead to: https://birb.js.org/explained/intents'
          )
        );
        this.preventReconnect(true);
        break;
  ...
  private identify(): void {
    if (this.status === Status.RESUMING) {
      if (Date.now() - this.lastResume > 5000) {
        ...
        this.client.warn(
          'attempting to reidentify in quick succession; scrapping old session and identifying normally...'
        );
        this.status = Status.CONNECTING;
      }
    }
