...
export default class MessageAttachment {
  ...
  constructor(
    ...
    this.filename =
      filename ?? (typeof fiddle == 'string' ? fiddle.split('/').pop() : null) ?? `unknown-${Date.now()}.attchmnt`;
  ...
  private static fromApiMessage(attachments?: any[] | null): MessageAttachment[] {
    ...
    return attachments.map(
      attach => new MessageAttachment(null, attach.filename, attach.id, attach.url, attach.proxy_url)
    );
  }
}
