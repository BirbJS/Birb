...
export default abstract class Cache {
   ...
   */
  constructor(options?: { maxSize?: number; maxAge?: number; checkInterval?: number; removeOldest?: boolean }) {
    options = options ?? {};
  ...
  makeSpace(options?: any): void {
    ...
    if (this.options.maxSize && this.cache.size >= this.options.maxSize) {
      if (options.allowOverflow) {
        process.emitWarning(
          new MemoryLeakWarning(
            'possible memory leak detected: overflow for cache has been permitted by options provided'
          )
        );
