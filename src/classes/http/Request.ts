  ...
  async make(files?: MessageAttachment[]) {
    ...
    if (body && body.code !== undefined) {
      ...
      throw new DiscordAPIError(
        `[${this.method}:${this.url}] ${body.code}: ${translateError(body.code, body.message)}`
      );
    }
    ...
    if (!res.statusCode || res.statusCode > 299) {
      ...
      throw new DiscordAPIError(
        `encountered HTTP error ${res.statusCode} whilst sending ${this.method} request to ${this.url}`
      );
    }
